...
return baseclass.extend({
	...
	load() {
		return Promise.all([
			...
			L.resolveDefault(fs.exec_direct('/usr/libexec/nlbwmon-action', ['download', '-g', 'mac', '-o', '-rx_bytes,-tx_bytes'], 'json'), { columns: [], data: [] })
		]).then(radios_networks_hints_nlbw => {
			const tasks = [];

			for (let i = 0; i < radios_networks_hints_nlbw[1].length; i++)
				tasks.push(L.resolveDefault(radios_networks_hints_nlbw[1][i].getAssocList(), []).then(L.bind((net, list) => {
					net.assoclist = list.sort((a, b) => { return a.mac > b.mac });
				}, this, radios_networks_hints_nlbw[1][i])));

			return Promise.all(tasks).then(() => {
				return radios_networks_hints_nlbw;
			});
		});
	},
	...
	renderHtml() {
		...
		const container_devices = E('table', { 'class': 'table assoclist devices-info' }, [
			...
			E('tr', { 'class': 'tr dashboard-bg' }, [
				...
				E('th', { 'class': 'th nowrap', 'width': '15%' }, [ _('SSID') ]),
				E('th', { 'class': 'th', 'width': '35%' }, [ _('Signal Strength') ]),
				E('th', { 'class': 'th nowrap', 'width': '25%' }, [ _('Transferred') + ' %s / %s'.format( _('Up.'), _('Down.')) ])
			])
			])
		]);
	...
	renderUpdateData(radios, networks, hosthints, nlbwData) {
		// 构建 nlbwmon 流量映射表：MAC -> { rx_bytes, tx_bytes }
		const nlbwMap = {};
		if (nlbwData && nlbwData.columns && nlbwData.data) {
			const colIdx = {};
			for (let i = 0; i < nlbwData.columns.length; i++) {
				colIdx[nlbwData.columns[i]] = i;
			}
			for (let i = 0; i < nlbwData.data.length; i++) {
				const row = nlbwData.data[i];
				const mac = String(row[colIdx['mac']] || '').toUpperCase();
				if (mac && mac !== '00:00:00:00:00:00') {
					nlbwMap[mac] = {
						rx_bytes: row[colIdx['rx_bytes']] || 0,
						tx_bytes: row[colIdx['tx_bytes']] || 0
					};
				}
			}
		}
		...
		for (let i = 0; i < networks.length; i++) {
			for (let k = 0; k < networks[i].assoclist.length; k++) {
				...
				// 使用 nlbwmon 数据替换 iwinfo 流量（NSS 硬件加速下 iwinfo 数据不准确）
				const macUpper = String(bss.mac).toUpperCase();
				let rxBytes = bss.rx.bytes;
				let txBytes = bss.tx.bytes;
				if (nlbwMap[macUpper]) {
					rxBytes = nlbwMap[macUpper].rx_bytes;
					txBytes = nlbwMap[macUpper].tx_bytes;
				}

				devicesTemp.push({
					bss: bss,
					name: name,
					ssid: networks[i].getActiveSSID(),
					q: q,
		...
		// 按 inactive 时间排序（越小越活跃，排前面）
		devicesTemp.sort((a, b) => (a.bss.inactive || 0) - (b.bss.inactive || 0));

		// 渲染排序后的设备
		for (let i = 0; i < devicesTemp.length; i++) {
			const dev = devicesTemp[i];
			this.params.wifi.devices.push(
				{
					...
					transferred : {
						...
						value: {
							// 表头：上行 / 下行
							// tx_bytes = 设备发送 = 上行，rx_bytes = 设备接收 = 下行
							rx: '%s'.format('%1024.2mB'.format(dev.txBytes)),
							tx: '%s'.format('%1024.2mB'.format(dev.rxBytes)),
						}
					}
				}
			);
	...
	render([radios, networks, hosthints, nlbwData]) {
		...
		this.renderUpdateData(radios, networks, hosthints, nlbwData);
